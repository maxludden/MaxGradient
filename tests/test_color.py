...
- triplet: Tuple[int, int, int] - The color value as an RGB tuple.
...
class TestColor(unittest.TestCase):
    ...
    def test_create_color_invalid_string(self):
        try:
            self.assertRaises(ColorParseError, Color, "#invalid")
            Color("invalid")

        except ColorParseError as e:
            self.assertEqual(str(e), "Unable to parse color: invalid")
    ...
    def test_create_color_valid_triplet(self):
        color = Color((255, 0, 0))
        self.assertEqual(color.original, "(255, 0, 0)")
        self.assertEqual(color.red, 255)
        self.assertEqual(color.green, 0)
        self.assertEqual(color.blue, 0)
        self.assertEqual(color.name, "red")
    ...
    def test_lighten_color(self):
        ...
        self.assertEqual(Color(lightened_color).blue, 127)

    # Tests that the darken method works correctly
    def test_darken_color(self):
        color = Color("#FF0000")
        darkened_color = color.darken(0.5)
        self.assertEqual(Color(darkened_color).red, 127)
        self.assertEqual(Color(darkened_color).green, 0)
        self.assertEqual(Color(darkened_color).blue, 0)

    # Tests that the contrast color for a black color can be obtained
    def test_get_contrast_for_black_color(self):
        color = Color("#000000")
        contrast = color.get_contrast()
        self.assertEqual(contrast, "#ffffff")

    # Tests that the color table can be generated
    def test_generate_color_table(self):
        color_table = Color.color_table()
        self.assertIsInstance(color_table, Columns)
