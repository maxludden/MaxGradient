...
class Gradient(Text):
    ...
    def __init__(
        ...
        *,
        ...
        gradient_substrings: Text = self.generate_gradient_substrings()
        console.line(2)
    ...
    def generate_indexes(self, verbose: bool = False) -> List[List[int]]:
        ...
        for count, index in enumerate(indexes):
            if verbose:
                log.success(f"[b white]Index {count}:[/]{', '.join([str(i) for i in index])}")
            else:
...
if __name__ == "__main__":  # pragma: no cover
    from highlighter import ColorHighlighter
    from rich.console import Console
    from maxgradient.theme import GradientTheme

    from maxgradient.log import Log
    gradient = Gradient(
        "Hello, World! This is a gradient!",
        colors=["red", "orange", "yellow", "lime", "cyan"],
        verbose=True
    )
    console.print(gradient, justify="center")
    console.print("Hello, World! This is a gradient!", justify="center")