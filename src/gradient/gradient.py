...
class SimpleGradient(Text):
    ...
    def __repr__(self) -> str:
        return f"Gradient({self._text!r}, \
    ...
    def style(self, style: StyleType) -> None:
        ...
        if style is None:
            ...
            if style in ["", "null", "none", "None"]:
                self._style = Style.null()
            self._style = Style.parse(style)
        else:
            raise TypeError(f"Style must be a string or Style, not {type(style)}")

    def generate_gradient(self) -> Generator[Span, None, None]:
        """
