...
class Rich:
    ...
    def get_triplets(cls) -> Tuple[ColorTriplet, ...]:
        return Rich.TRIPLETS

    @classmethod
    @lru_cache(maxsize=10, typed=False)
    def get_color(cls, color: str) -> ColorTriplet:
        """Parse rich colors for color"""
        for group in [cls.get_names(), cls.get_hex(), cls.get_rgb(), cls.get_triplets()]:
            if color in group:
                index = group.index(color)
                rgb = cls.get_rgb()[index]
                triplet: ColorTriplet = cls.rgb_to_triplet(rgb)
                return triplet

        raise ValueError(f"Invalid color: {color}")

    @staticmethod
    def rgb_to_triplet(rgb: str) -> ColorTriplet:
    ...
    def color_table(cls) -> Table:
        ...
        color_table.add_column("ColorTriplet", justify="left", style="bold")
        for index, _ in enumerate(HEX):
            ...
            color_triplet = cls.rgb_to_triplet(color_rgb)
            color_table.add_row(
                ...
                Text(str(color_triplet), style=color_hex),
            )
        return color_table
